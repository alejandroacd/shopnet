...
export const MpProvider = ({ children }) => {
    ...
    const mpAccessTokenRequest = async (code) => {
        console.log(theCode)
        let theCode = code;
        await axios.post(`https://api.mercadopago.com/oauth/token?client_id=72333279858722&client_secret=RQK2n18b4ngiLcibxQrM08ATGYfhipK1&grant_type=authorization_code&redirect_uri=https://theshopnet.netlify.app/successfullBinding&code=${theCode}`)
            .then(res => {
